...
for year in range(2007, 2023):
    files.append(pd.read_csv(f"data/Seenberichtsdaten Jahr {year}.csv", encoding='ANSI', sep=";", decimal=','))
...
watter_temp_lake1 = data_lake1[data_lake1.PARAMETERBEZEICHNUNG == 'Wassertemperatur'].reset_index(drop=True)

print(watter_temp_lake1)

for xi, yi_min, yi_max in zip(watter_temp_lake1.JAHR, watter_temp_lake1.MIN, watter_temp_lake1.MAX):
    plt.vlines(xi, ymin=yi_min, ymax=yi_max, colors='r', linewidth=2, label='Min/Max Values')
