...
def setup():
    global root, output_label, write_button, autor, titel, auflage, preis, antwort, mod
    mod = m.Db_Zugriff()
    root = tk.Tk()
    ...
    label4 = tk.Label(buch_frame, text='Auflage: ')
    auflage = tk.IntVar()
    titel_entry = ttk.Entry(buch_frame, textvariable=auflage)
    label4.grid(column=0, row=3, padx=10, sticky=tk.W)
    titel_entry.grid(row=3, column=1, sticky=tk.W, padx=10)
    
    label5 = tk.Label(buch_frame, text='Preis: ')
    preis = tk.DoubleVar()
    titel_entry = ttk.Entry(buch_frame, textvariable=preis)
    label5.grid(column=0, row=4, padx=10, sticky=tk.W)
    titel_entry.grid(row=4, column=1, sticky=tk.W, padx=10)
...
def insert_buch():
    global autor, titel, antwort, mod, auflage, preis
    rows = mod.insertBuch(mod.get_next_buchnr(), titel.get(), autor.get(), auflage.get(), preis.get())
